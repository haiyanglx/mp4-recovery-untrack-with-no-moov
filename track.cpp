...
void Codec::parse(Atom *trak, vector<int> &offsets, Atom *mdat) {
	...
	if(entries != 1)
		...
		LOG("Multiplexed stream! Not supported");
...
//				nal_type != 8 && nal_type != 9 && nal_type != 10 && nal_type != 11 && nal_type != 12) {
			...
			LOG("avc1: no match because of nal type:%d",nal_type);
#endif
			return false;
		}
		...
		if(start[0] == 0) {
			...
			LOG("avc1: Match with 0 header");
#endif
			return true;
		}
#ifdef VERBOSE
		//cout << "avc1: failed for not particular reason\n";
		LOG("avc1: failed for not particular reason");
#endif
		return false;
		...
		if(s > 1000000) {
			...
			LOG("mp4a: Success because of large s value");
#endif
			return true;
		}
		...
		if((start[4] == 0xee && start[5] == 0x1b) ||
			...
			LOG("mp4a: Success because of horrible hack");
			return true;
		}
		...
		if(start[0] == 0) {
			...
			LOG("Failure because of NULL header");
			return false;
#endif
		}
#ifdef VERBOSE
		//cout << "Success for no particular reason....\n";
		LOG("Success for no particular reason...");
#endif
		return true;
		...
		LOG("This audio codec is EVIL, there is no hope to guess it...");
		exit(0);
		return true;
	} else if(name == "apcn") {
		...
		LOG("Sowt is just  raw data, no way to guess length (unless reliably detecting the other codec start)");
		return false;
	}
...
int golomb(uint8_t *&buffer, int &offset) {
	...
	while((*buffer & (0x1<<(7 - offset))) == 0) {
		...
		if(count > 20) {
			...
			LOG("Failed reading golomb: too large!");
			return -1;
		}
	}
...
H264sps parseSPS(uint8_t *data, int size) {
	...
	if(cnt != 1) {
		...
		LOG("Not supporting more than 1 SPS unit for the moment, might fail horribly!");
	}
	for (i = 0; i < cnt; i++) {
		//nalsize = AV_RB16(p) + 2;
...
bool getNalInfo(H264sps &sps, uint32_t maxlength, uint8_t *buffer, NalInfo &info) {
	...
	LOG("Frame num:: %d",info.frame_num);
...
int Codec::getLength(unsigned char *start, int maxlength, int &duration) {
	if(name == "mp4a") {
		...
		LOG("Duration=%d",frame->nb_samples);
		av_freep(&frame);
		return consumed;
  ...
  *     AVFrame *frame = avcodec_alloc_frame();
		...
		LOG("Unfortunately I found no way to guess size of mp4v packets. Sorry");
		return -1;
		...
		while(1) {
			...
			default:
				if(seen_slice) {
					...
					LOG("New access unit since seen picture");
					return length;
				}
				break;
			}
...
void Track::parse(Atom *t, Atom *mdat) {
	...
	if(!mdhd)
		...
		LOG("No mdhd atom: unknown duration and timescale");
	...
	if(avcodec_open2(codec.context, codec.codec, NULL)<0)
		...
		LOG("Could not open codec=%s",codec.context->codec_name);
...
vector<int> Track::getSampleTimes(Atom *t) {
	...
	if(!stts)
		...
		LOG("Missing sample to time atom");
...
vector<int> Track::getSampleSizes(Atom *t) {
	...
	if(!stsz)
		...
		LOG("Missing sample to sizeatom");
...
vector<int> Track::getSampleToChunk(Atom *t, int nchunks){
	...
	if(!stsc)
		...
		LOG("Missing sample to chunk atom");
