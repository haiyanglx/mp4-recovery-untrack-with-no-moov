...
void Atom::parse(File &file) {
    ...
    if(isParent(name) && name != string("udta")) { //user data atom is dangerous... i should actually skip all
        ...
        if(content.size() < length -8)
            ...
            LOG("Failed reading atom content: %s",name);
    }
}
...
void Atom::print(int offset) {
    ...
    if(name == string("mvhd") || name == string("mdhd")) {
        ...
        LOG("Timescale:: %d,Duration=%d",readInt(12),readInt(16));
        ...
        LOG("Trak:: %d,Duration=%d",readInt(12),readInt(20));
        ...
        LOG("Entries: %d,codec:=%s",readInt(4),type);
		...
		for(int i = 0; i < entries && i < 30; i++)
            ...
            LOG("samples:: %d,for:=%d",readInt(8 + 8*i),readInt(12 + 8*i));
        ...
        for(int i = 0; i < entries && i < 10; i++)
            ...
            LOG("chunk:: %d,nsamples:=%d,id:=%d",readInt(8 + 12*i),readInt(12 + 12*i),readInt(16 + 12*i));
        ...
        LOG("Sample size:: %d,Entries:=%d",sample_size,entries);
        if(sample_size == 0) {
            for(int i = 0; i < entries && i < 10; i++)
                //cout << indent << " Size " << readInt(12 + i*4) << endl;
                LOG("Size: %d",readInt(12 + i*4));
        }
...
unsigned char *BufferedAtom::getFragment(int64_t offset, int64_t size) {
    if(offset < 0)
        ...
        LOG("Offset set before beginning of buffer");
