...
void Mp4::open(string filename) {
	...
	if(root->atomByName("ctts"))
		...
		LOG("Composition time offset atom found. Out of order samples possible");

	if(root->atomByName("sdtp"))
		cerr << "Sample dependency flag atom found. I and P frames might need to recover that info." << endl;
		LOG("Sample dependency flag atom found. I and P frames might need to recover that info");

	Atom *mvhd = root->atomByName("mvhd");
	if(!mvhd)
		//throw string("Missing movie header atom");
		LOG("Missing movie header atom");
	...
	if(error != 0)
		...
		LOG("Could not parse AV file =%s",filename.c_str());
...
void Mp4::makeStreamable(string filename, string output) {
	...
	if(mdat->start > moov->start) {
		...
		LOG("FIle is already streamable");
		return;
	}
	...
	LOG("OLD:=%d,new: %d",old_start,new_start);
...

	{
		...
		if(!file.create(output))
			...
			LOG("Could not create file for writing:: %s",output.c_str());
...
*/
	...
	for(unsigned int i = 0; i < tracks.size(); i++) {
		...
		LOG("Track duration: %d,movie timescale:%d,track timescale:%d",track.duration,timescale,track.timescale);
	...
	if(!file.create(filename))
		...
		LOG("Could not create file for writing: %s",filename.c_str());
...
void Mp4::analyze() {
	...
	for(unsigned int i = 0; i < tracks.size(); i++) {
		...
		LOG("Track codec: %s",track.codec.name.c_str());

		//cout << "Keyframes  " << track.keyframes.size() << endl;
		LOG("Keyframes %d",track.keyframes.size());
		...
		for(unsigned int i = 0; i < track.offsets.size(); i++) {
			...
			if(!matches) {
				...
				LOG("Match failed! Hit enter for next match");
				getchar();
			}
			//assert(matches);
			//cout << "Length: " << length << " true length: " << track.sizes[i] << endl;
			LOG("Length:%d,true length:%d",length,track.sizes[i]);
...
void Mp4::repair(string filename) {
	...
	while(offset < mdat->contentSize()) {
		...
		if(start[4] == 'm' && start[5] == 'o' && start[6] == 'o' && start[7] == 'v') {
			...
			LOG("Skipping moov atom:::%d",swap32(begin));
			offset += swap32(begin);
			continue;
		}
		...
		for(unsigned int i = 0; i < tracks.size(); i++) {
			...
			if(length < -1 || length > 1600000) {
				...
				LOG("Invalid length:%d,Wrong match in track:%d",length,i);
				continue;
			}
